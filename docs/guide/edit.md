...
!!! warning "注意"
    ...
    如果存在较为复杂的格式需求，我们建议在其他编辑器中排版完成后，将带格式文本粘贴到编辑器中。关于工具的选择，参阅[资源](../resources.md)。
...
!!! warning "注意"
    ...
    如果你将包含图片的富文本内容粘贴到编辑器中，并且其中的图片是以外部链接的形式存储，则这些图片并不会被自动上传到少数派的服务器上；读者阅读时，浏览器也会直接从其原始链接下载图片。这可能导致图片加载缓慢或失败。
    
    因此，我们建议在这种情况下手动上传一遍图片。
...
!!! xref "参阅"
    ...
    要了解转换和压缩图片的工具，参见[工具和技巧 § 转换和压缩图片](/resources/software#img)。
...
点击正文上方的封面图区域，可以上传封面图。

封面图的标准尺寸为 1420 × 708 像素，关键内容应放置在中央 708 × 708 像素的区域。如果上传的封面图不符合上述尺寸，可以在上传时根据提示裁剪。如果选择「保留原图」，系统会在显示时自动裁剪。

!!! xref "参阅"

    要下载标记有封面图尺寸和安全区域的 Figma 模板，参见[工具和技巧 § 封面图](/resources/software#img)。

对于没有封面图的文章，系统会通过截取标题生成一个临时封面。
